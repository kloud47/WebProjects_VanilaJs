...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === Number(inputLoginPin.value)) {
    ...
    //// Initializing dates: (new Intl.DateTimeFormat(locales, options)):///////////
    const now = new Date();
    const options = {
      hour: 'numeric',
      minute: 'numeric',
    ...
    /// (more country date formats in ISO language code table):
...
btnTransfer.addEventListener('click', function (e) {
  ...
  if (
    ...
    // Add Transfer date:
    currentAccount.movementsDates.push(new Date().toISOString()); /// Consverts string into ISO date format
...
labelBalance.addEventListener('click', function() {
  [...document.querySelectorAll('.movements__row')].forEach(function(row, i) {
    row.style.backgroundColor = (i % 2 === 0 && !color)?  'orangered' : 'white';
  });
  color = !color;
});
